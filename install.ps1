...
Function Import-Containers {
    Write-Host "Configuration complete.`n"
    Try {
        Write-Host "Importing containers..."
        docker load -i pulsar_web.tar
        docker load -i pulsar_sandbox.tar
        docker load -i pulsar_queue.tar
        docker load -i pulsar_db.tar 
        Write-Host "Import finished!"
    }
    Catch {
        Write-Error "Failed to import docker containers."
        exit
    }
}
