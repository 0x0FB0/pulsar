...
class TLDInstance(models.Model):
    """
    Helper model for TLD Expansion scan.
    """
    id = models.UUIDField(primary_key=True, unique=True, default=uuid.uuid4, editable=False)
    fqdn = models.CharField(max_length=255, default='none')
    asset = models.ForeignKey(AssetInstance, related_name='tlds', on_delete=models.SET_NULL,
                              null=True, blank=True)
...
class DomainInstance(models.Model):
    ...
    def get_siblings(self):
        str_ips = [ip.ip for ip in IPv4AddrInstance.objects.filter(id__in=self.ips.all())]
        sib_ips = IPv4AddrInstance.objects.filter(last_task=self.last_task, ip__in=str_ips)
        sib_doms = DomainInstance.objects.filter(last_task=self.last_task, ips__in=sib_ips)
        siblings = [dom.fqdn for dom in sib_doms].remove(self.fqdn)
        if siblings is None:
            siblings = []
        elif type(siblings) != list:
...
class HandMadePlugin(models.Model):
    ...
    script = models.TextField(blank=True, default='#!/bin/bash\n\n# Put Your script contents here.\n'
                              + '# Vulnerability is triggered by any output generated by this script.\n'
                              + '# Use $DOM_SVCS and $DOM_FQDN environmental variables.\n'
                              + '# DOM_SVC=22/SSH,80/HTTP,8080/HTTP DOM_FQDN=example.com'
                              + '# Example:\n'
                              + '# echo $DOM_SVC | tr "," "\n" | grep HTTP | cut -d "/" -f 1 | while read svc'
                              + '# do curl "http://${DOM_FQDN}:${svc}/vuln.php" 2>&1 | grep VULNERABILITY'
                              + '# done\n\n')
